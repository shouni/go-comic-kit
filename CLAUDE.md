...
- **`internal/operations`** — one file per operation (`outline.go`, `chapter.go`, `design.go`, `panel.go`, `page.go`), plus purpose-named shared helpers (`gen_opts.go`, `json_response.go`, `source_text.go`, `character_roster.go`, `image_output.go`, `seed.go`, `panel_desc.go` — no grab-bag `common.go`). `image_output.go`'s `renderImage` owns the generate → save → `GenerationRecord` tail shared by panels, pages and design sheets; the three operations keep only what differs (prompt assembly, where the file goes, the log line). Operations (`OutlineRunner`, `ChapterScriptRunner`, etc. — the `Runner` suffix stays on the types even though the package is `operations`) depend on narrow interfaces (`ImageFusionGenerator`, `DesignImageGenerator`, `gemini.ContentGenerator`), not concrete layout types, so tests use lightweight fakes. Building an image request is now pure — `buildGenerateRequest` / `buildComposeRequest` take neither a context nor the state, because collecting references no longer performs I/O.
- **`internal/layout`** — aspect-ratio constants and normalization (`types.go`). It used to also hold `ComicComposer`, which pre-uploaded reference images to the File API and cached the resulting URIs behind double-checked locking + singleflight. gemini-image-kit v1.12.2 moved that decision into `ports.ReferenceResolver` (`GeminiImageCore.ResolveReference`: Vertex + `gs://` passes through, Gemini API uploads once via a singleflighted, cached `EnsureUploaded`), so the operations now pass `ImageURI{ReferenceURL: url}` and the kit resolves it. Do not reintroduce a pre-upload pass here — it would duplicate the kit's cache and re-open the drift that had this copy and go-veo-orchestrator's diverge.
