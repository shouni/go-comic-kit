...
- **`internal/prompts`** — kit-embedded prompt implementations, **all five** overridable via `workflow.Args` (nil = kit default): `ScriptPrompts` for outline/chapter (`go:embed templates/{outline,chapter}/*.md`; dropping a new `.md` file adds a new mode, filename = mode name, like ap-comp's visual_mode) implementing `ports.OutlinePrompt` / `ports.ChapterScriptPrompt`, `DefaultDesignPrompt` (plain Go, no templates — subject list and layout are structural, not template-friendly) implementing `ports.DesignSheetPrompt`, and `DefaultPanelPrompt` / `DefaultPagePrompt` implementing `ports.PanelPrompt` / `ports.PagePrompt`.
- **The built-in panel/page prompts are deliberately terse.** They carry only what breaks the *format* if omitted — reference-image ordering, panel count, reading order, no lettering, five fingers — and nothing about art direction. Product-specific wording (layout maps, balloon shapes, ALL-CAPS production directives) lives in the consuming app, because that text is iterated on per work and should not require a kit release. The elaborate page prompt this kit used to carry is in git history before the prompt-DI commit; lift it into ap-comic rather than re-adding it here. The outline/chapter mode used is persisted in `MangaState.ScriptMode` so regeneration uses the same prompt.
